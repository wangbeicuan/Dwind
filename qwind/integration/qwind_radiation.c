...
gsl_integration_cquad_workspace *w5 = NULL;//
gsl_integration_cquad_workspace *w6 = NULL;//
...
gsl_function *gsl_integrate_simplesed_phi_phi_d = NULL;
gsl_function *gsl_integrate_simplesed_phi_r_d = NULL;
...
double integrate_simplesed_z_phi_d(double phi_d, void *params)//返回被积分函数
{
    ...
    hongyi=1+sqrt(1/PARAMS->r_d)*PARAMS->r/sqrt(delta)*sin(phi_d);//红移修正
    integrand_value = (1/pow(hongyi,4) )*1/ pow(delta,2.);
...
double integrate_simplesed_r_phi_d(double phi_d, void *params)
{
    ...
    delta = pow(PARAMS->r,2.) + pow(PARAMS->z,2.) + pow(PARAMS->r_d,2.) - 2 * PARAMS->r * PARAMS->r_d * cos(phi_d);
    hongyi=1+sqrt(1/PARAMS->r_d)*PARAMS->r/sqrt(delta)*sin(phi_d);//红移修正
    integrand_value = cos_gamma / pow(delta,2.)*(1/pow(hongyi,4));
    return integrand_value;
}
...
double integrate_simplesed_r(parameters *params)
{
    ...
    gsl_integration_cquad(gsl_integrate_simplesed_r_r_d,
                          ...
                          &error,
                          &n_eval);
    result =  PARAMS->z * result;
    return result;
}

double integrate_simplesed_phi_phi_d(double phi_d, void *params)
{
    double integrand_value = 0;
    double sin_gamma;
    double delta;
    double hongyi;
    sin_gamma = -1* PARAMS->r_d * sin(phi_d);
    delta = pow(PARAMS->r,2.) + pow(PARAMS->z,2.) + pow(PARAMS->r_d,2.) - 2 * PARAMS->r * PARAMS->r_d * cos(phi_d);
    hongyi=1+sqrt(1/PARAMS->r_d)*PARAMS->r/sqrt(delta)*sin(phi_d);//红移修正
    integrand_value = sin_gamma / pow(delta,2.)*(1/pow(hongyi,4));
    return integrand_value;
}
double integrate_simplesed_phi_r_d(double r_d , void *params)
{
    double result, error;
    PARAMS->r_d = r_d;
    gsl_integration_cquad(gsl_integrate_simplesed_phi_phi_d,
                          0.,
                          2*M_PI,
                          0,
                          PARAMS->epsrel,
...
double integrate_simplesed_phi(parameters *params)
{
    PARAMS = params;
    double result, error;
    gsl_integration_cquad(gsl_integrate_simplesed_phi_r_d,
                          PARAMS->r_min,
                          PARAMS->r_max,
                          0,
                          PARAMS->epsrel,
...
void initialize_integrators()
{
    ...
    gsl_integrate_simplesed_phi_phi_d = malloc(sizeof(gsl_function));
    gsl_integrate_simplesed_phi_phi_d->function = &integrate_simplesed_phi_phi_d;
    gsl_integrate_simplesed_phi_phi_d->params = NULL;

    gsl_integrate_simplesed_phi_r_d = malloc(sizeof(gsl_function));
    gsl_integrate_simplesed_phi_r_d->function = &integrate_simplesed_phi_r_d;
    gsl_integrate_simplesed_phi_r_d->params = NULL;
    ...
    w5 = gsl_integration_cquad_workspace_alloc(n_eval);
    w6 = gsl_integration_cquad_workspace_alloc(n_eval);
}
...
int main(void)
{
    ...
    printf("%e\n",integrate_simplesed_phi(&params));
    return 0;
}
